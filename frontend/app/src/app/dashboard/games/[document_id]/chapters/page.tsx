...
export default function ChapterSelectionPage({ params }: PageProps) {
  ...
  const fetchChapterData = useCallback(async () => {
    ...
    try {
      ...
      let chaptersRes;
      try {
        chaptersRes = await axios.get(`/api/document-content/documents/${documentId}/chapters`);
      } catch (err: unknown) {
        const status = axios.isAxiosError(err) ? err.response?.status : undefined;
        if (status === 404) {
          throw new Error("Game or study notes not found.");
        }
        throw new Error(`Failed to load chapters (Code ${status || 'network error'})`);
      }
      ...
      try {
        ...
        console.error("Failed to load matching game name:", e);
      }
    } catch (err: unknown) {

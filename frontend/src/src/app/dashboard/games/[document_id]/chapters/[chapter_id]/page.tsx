...
export default function LevelsPage({ params }: PageProps) {
  ...
  const fetchLevelsData = useCallback(async () => {
    ...
    try {
      ...
      const levelsRes = await fetch(`/api/document-content/chapters/${chapterId}/sub-chapters`, {
        headers: {
      ...
      if (!levelsRes.ok) {
        ...
        throw new Error(`Failed to load levels (Code ${levelsRes.status})`);
      }
  ...
  const activeChapterIndex = activeChapter?.chapter_index !== null && activeChapter?.chapter_index !== undefined
    ? activeChapter.chapter_index
    : null;
              ...
              {levels.map((level, idx) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      alert(`Launching Level ${levelNum}: ${level.title || 'Untitled'}`);
                    }}

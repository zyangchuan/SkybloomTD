...
export default function DashboardPage() {
  ...
  const fetchTaskStatus = useCallback(async (taskId: string, docId: string, currentDocsList?: DocumentSummary[]) => {
    ...
    try {
      ...
      const res = await fetch(`/api/document-content/tasks/${taskId}/status`, {
        headers: {
  ...
  const fetchDocuments = useCallback(async (showLoading = true) => {
    ...
    try {
      const token = session.access_token;
      const res = await fetch('/api/document-content/documents', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleDeleteDocument = useCallback(async (docId: string) => {
    ...
    try {
      const token = session.access_token;
      const res = await fetch(`/api/document-content/documents/${docId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }

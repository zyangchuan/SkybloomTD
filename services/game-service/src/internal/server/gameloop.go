	...
	func (s *Server) runGameLoop(ctx context.Context, conn *websocket.Conn, writeMu *sync.Mutex, runtime runtimeSession, actions <-chan clientAction, done chan<- struct{}) {
		...
		for {
			...
			case action := <-actions:
				...
				case setSpeedAction:
					m := action.SpeedMultiplier
					if m < 1 {
						m = 1
					}
					runtime.speedMultiplier = m
				default:
					if action.Result != nil {
			...
			case now := <-ticker.C:
				...
				// add time number of ticks in time intervals
				ticks := runtime.speedMultiplier
				if ticks < 1 { 
					ticks = 1
				}
				var allEvents []GameEvent
    			for i := 0; i < ticks; i++ {
       				 allEvents = append(allEvents, advanceRuntimeTick(&runtime, now)...)
				...
				if err := writeWebsocketJSON(conn, writeMu, Message{Type: "game.state", Data: gameStateFromRuntime(runtime, runtime.session.UpdatedAt, nil, allEvents)}); err != nil {

...
def extract_heading_candidates(lines: list[str]) -> list[HeadingCandidate]:
    ...
    for index, heading in enumerate(headings):
        previous_title = headings[index - 1]["title"] if index > 0 else None
        next_title = headings[index + 1]["title"] if index + 1 < len(headings) else None
        end_line = (
            headings[index + 1]["line_number"] - 1
...
def chunk_chapters(
    ...
    for chapter in chapters:
        chapter_lines = lines[chapter.start_line - 1 : chapter.end_line]
        chunk_start_offset = 0
        chunk_index = 1

        while chunk_start_offset < len(chapter_lines):
            selected_lines: list[str] = []
            selected_chars = 0
            offset = chunk_start_offset

            while offset < len(chapter_lines):
                line = chapter_lines[offset]
                line_chars = len(line) + 1
                if selected_lines and selected_chars + line_chars > max_chars:
                    break
            ...
            start_line = chapter.start_line + chunk_start_offset
            end_line = chapter.start_line + offset - 1
            ...
            if offset >= len(chapter_lines):
                break

            chunk_start_offset = max(offset - overlap_lines, chunk_start_offset + 1)
